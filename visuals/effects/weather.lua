...
local function spawn_particle(cx, cy, draw_dist)
...
function weather:load(cx, cy)
	for _ = 1, render_cfg.particle_count do
		local p = spawn_particle(cx, cy, render_cfg.draw_distance)
...
function weather:update(dt, cx, cy)
	...
	for i, p in ipairs(self.particles) do
		...
		if hit or p.z < map.min_z or out_x or out_y then
			...
			if p.linger <= 0 then
				self.particles[i] = spawn_particle(cx, cy, draw_dist)
			end
		end
	end
end
...
function weather:set_mode(weather_mode)
	if weather_mode == modes.normal then
		self.particles = {}
	end
	weather.mode = modes[weather_mode]
end
